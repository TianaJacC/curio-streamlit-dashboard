...
# ==============================================================================
# 皇家郵政信鴿 ‧ 探險家羽毛信投遞 (No-PII 擬人化回饋機制)
# ==============================================================================
...
if hasattr(st, "dialog"):
    ...
    def pigeon_dispatch_modal(current_token: str):
        ...
        cat = st.radio(
            "請為這封羽毛信選擇封蠟類別：",
            ["📜 羊皮紙卷翻頁卡頓（操作不順）", "📷 鏡頭微光感應微弱（rPPG問題）", "💡 給閣長與信哥的靈感悄悄話"],
            index=0
        )
        msg_body = st.text_area("羽毛信內容：", placeholder="咕咕！請告訴信哥您在夢境裡遇到的狀況...", height=90)
        
        if st.button("🕊️ 繫上羽毛信，讓信哥起飛！", use_container_width=True):
            if msg_body.strip():
                save_feedback("探險家", current_token, cat, msg_body)
                st.success("✨ 咕咕！信哥已拍打翅膀起飛，羽毛信已安全送達夢境管理處檔案庫！")
                time.sleep(1.2)
                st.rerun()
            else:
                st.warning("⚠️ 咕咕！羽毛信還是空白的，請寫下一點點線索再讓信哥出發喔！")
# ==============================================================================
